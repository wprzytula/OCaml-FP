...
(*      Reviewer: Aleksandra Martyniuk gr. 7 *)
...
(* Zwraca [true] jeśli dana kolejka jest pusta. *)
...
(* order_asc : 'a queue -> 'a queue -> 'a queue * 'a queue *)
(* Zwraca krotkę kolejek taką, że pierwsza w kolejności    *)
(* kolejka ma w korzeniu element o niższym priorytecie.    *)
let order_asc (q1, q2) =
    let Node(_, val_q1, _, _) = q1
    and Node(_, val_q2, _, _) = q2
    in if val_q1 < val_q2 then (q1, q2) else (q2, q1)

        
(* join : 'a queue -> 'a queue -> 'a queue            *)
...
let rec join q1 q2 =
    ...
    | (Null, _) -> q2
    | _ ->
        let (q1, q2) = order_asc (q1, q2)
        in let Node(qleft, root_val, qright, _) = q1
        in let sh_path_qleft =
            match qleft with
            | Node(_, _, _, x) -> x
            | Null -> 0
        in let Node(_, _,  _, sh_path_q3) as q3 = join qright q2
        in 
            if sh_path_qleft  >= sh_path_q3
            then Node(qleft, root_val, q3, sh_path_q3 + 1)
            else Node(q3, root_val, qleft, sh_path_qleft + 1)


(*  new_queue : 'a -> 'a queue     *)
(*  new_queue el zwraca kolejkę    *)
(*  zawierającą jeden element el   *)
let new_queue el =
    Node(Null, el, Null, 1)
    
    
(*  add : 'a -> 'a queue -> 'a queue          *)
(*  [add e q] zwraca kolejkę powstałą z       *)
(*  dołączenia elementu [e] do kolejki [q]    *)
let add el q =
    join q (new_queue el)
    
    
(*  delete_min : 'a queue -> 'a * 'a queue        *)
(*  Dla niepustej kolejki [q], [delete_min q]     *)
(*  zwraca parę [(e,q')] gdzie [e]                *)
(*  jest elementem minimalnym kolejki [q]         *)
(*  a [q'] to [q] bez elementu [e].               *)
(*  Jeśli [q] jest puste podnosi wyjątek [Empty]. *)
let delete_min q =
    match q with
    | Null -> raise Empty
    | Node(q1, value, q2, _) -> (value, join q1 q2)
...
(* Returns true if ALL values from q are taken out
   in the order given in l *)
let test q l =
    try
